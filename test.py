...
class Tests(unittest.TestCase):
    ...
    def _test_folder_structure(self, project_number):
        ...
        self.assertTrue(miniproject_path.exists(),
                        f"No folder Miniproject_{project_number} found at {project_path}")

        for file in ["__init__.py", "model.py"]:
            with self.subTest(f"Checking file {file} for project {project_number}"):
                self.assertTrue((miniproject_path / file).exists(),
                                f"No file {file} found at {miniproject_path}")

        for file in [f"Report_{project_number}.pdf", "bestmodel.pth"]:
            if not (miniproject_path / file).exists():
                warn(
                    f"Miniproject folder {project_number} does not contain a {file} file")
    ...
    def test_framework_block(self):
        ...
        with self.subTest("Testing convolution"):
            ...
            conv = Conv2d(3, 3, 3)
            self.assertTrue(
                torch.allclose(conv(x), F.conv2d(x, conv.weight, conv.bias)))
        ...
        with self.subTest("Testing sequential"):
            ...
            self.assertTrue(
                torch.allclose(seq(x), F.conv2d(x, conv.weight, conv.bias).sigmoid()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--project-path', help='Path to the project folder',
                        required=True)
    parser.add_argument('-d', '--data-path', help='Path to the data folder',
                        required=True)
